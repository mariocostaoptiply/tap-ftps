...
def sample_file(conn, table_spec, f, sample_rate, max_records, config):
    ...
    if decryption_configs:
        ...
        file_handle = conn.get_file_handle(f, decryption_configs)
        # Update filepath to decrypted file path if decryption was used
...
def pick_datatype(type_count):
    """
    Determines the datatype for a column based on type counts.
    
    Returns 'date-time' if any date-time values are found.
    Otherwise returns 'string' to preserve leading zeros in numeric strings
    (e.g., product codes like "01234" should remain as strings).
    """
