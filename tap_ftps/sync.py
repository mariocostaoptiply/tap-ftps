        ...
        for reader in readers:
            ...
            with Transformer() as transformer:
                for row in reader:
                    ...
                    # Ensure _sdc_extra is always an array (schema requirement)
                    if csv_handler.SDC_EXTRA_COLUMN in row:
                        extra_value = row[csv_handler.SDC_EXTRA_COLUMN]
                        if extra_value is None:
                            row[csv_handler.SDC_EXTRA_COLUMN] = []
                        elif isinstance(extra_value, str):
                            # If it's a string, convert to array with one element
                            row[csv_handler.SDC_EXTRA_COLUMN] = [extra_value]
                        elif not isinstance(extra_value, list):
                            # If it's not a list, convert to list
                            row[csv_handler.SDC_EXTRA_COLUMN] = [str(extra_value)]
                    else:
                        # If _sdc_extra doesn't exist, set it to empty array
                        row[csv_handler.SDC_EXTRA_COLUMN] = []
